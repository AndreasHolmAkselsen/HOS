function [W_lin,W_nl] = phiComponentsHOS(phiS,eta,param)
    ...
    for n = 1:M
        ...
        hphi_jn(:,n) = fftPad(cosfft(phi_jni(:,n,1)),N);% NB
        for i = 1:(M-n+1)
            phi_jni(:,n,i+1) = real(icosfft(fftPad(H_ji(:,i+1).*hphi_jn(:,n),Nd))); 
        end        
        %compute W^(n)
        for i = 0:n-1
...
%     W_jn   = real(W_jn);   
    ...
    W_nl   = sum(W_jn(:,2:M),2);
    if param.DO_PADDING
        W_lin = icosfft(fftPad(cosfft(W_lin),N));
        W_nl = icosfft(fftPad(cosfft(W_nl),N));
    end
%     hphi = sum(hphi_jn,2);
