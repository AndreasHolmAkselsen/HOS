...
exportPrefix =  'closedNewdimL_zeroMap_';
exportPath = './figures/';
...
dim.L  = (map.xi(2)-map.xi(1))*(nx-1)/pi; % note different scaling for closed domain!
dim.t = sqrt(dim.L/g);
dim.phi = sqrt(dim.L^3*g);
dim.U = dim.L/dim.t;
param.dim = dim;
...
if RK4dt~=0
    ...
    ODEoptions = odeset('RelTol',relTolODE,'InitialStep',initialStepODE/dim.t);
