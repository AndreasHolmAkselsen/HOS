...
% Flat bottom:
%%%%%
xx_b = [];  %2.7726  % xi-coordinate of "begining of" edge
H = [1]; % plateau levels
theta = []; % slope angles (positive values)
%%%%%

% Flat bottom still using conformal map:
%%%%%
% xx_b = [0];  %2.7726  % xi-coordinate of "begining of" edge
% H = [1,1]; % plateau levels
% theta = [0]; % slope angles (positive values)
%%%%%
...
% % specifying period
T = 2.5; omega=2*pi/T; k1 = findWaveNumbers(omega,H(1),0,0);
...
fileName = sprintf('%s%s_%s_T%.2f_ka%.2g_M%d_H%stheta%sNw%d_dt%.3gT_nx%d_pad%d_ikCut%.4g_Md%.2g_r%.2g',exportPrefix,boundaryType,INIT_WAVE_TYPE,T,ka,param.M,Hstr,thetaStr,NWaves,NT_dt,nx,param.DO_PADDING,param.iModeCut,param.kd__kmax,rDamping); fileName(fileName=='.')='p';
if DO_EXPORT
    copyfile('./proto_num.m',[exportPath,'/m/',fileName,'.m']) 
end
...
%         xxIP = xxIP_far;
        xxIP_near = 0; zzRoots( isnan(zzRoots))=0;
    end
   ...
   assert(isempty(theta) && isscalar(H));
   map.xi = x;
   map.zzDepth = H;
   varphiS0 = phiS0;
   eta0_xiReg = h0;
