...
exportNameCurr = sprintf('%sL%g',exportPrefix,L); exportNameCurr(exportNameCurr=='.')='p';
...
%     fprintf('c_p = %.3gm/s, max |U(0)| = %.3gm/s, fraction: %.3g\n',c_p,max(abs(df(x))),max(abs(df(x)))/c_p)
    ...
    clear x
    if DO_EXPORT
        savefig(hf_c,[exportPath,'/curr_',exportNameCurr]);
        export_fig(hf_c,[exportPath,'/curr_',exportNameCurr],'-pdf');
        savefig(hf_Usurf,[exportPath,'/Usruf_',exportNameCurr]);
        export_fig(hf_Usurf,[exportPath,'/Usruf_',exportNameCurr],'-pdf');
    end
...
nonLinRamp = @(t) max(0,1-exp(-(t/TRamp)^rampExponent));
...
switch plotType
    case 'tight'
        ...
        dz__a = 1;
        nz = a__height/dz__a;
        zz = dz__a*(0:nz-1)'*max(abs(eta(1,:)));
        t_ip = linspace(0,t(end),nz)';
    ...
    case 'subplot'
        ...
        nPannel = length(t);
        for iP = 1:nPannel
            subplot(nPannel,1,nPannel-iP+1), plot(res.x,eta(iP,:),'k');hold on
            ylabel(sprintf('t = %.2fs',t(iP)))
            set(gca,'XTick',[],'XAxisLocation','origin');
            box off; grid on;
            % plot marker
            xDrift = mod(t(iP)*c_p,L);
            yMarker =  interp1(res.x,eta(iP,:),xDrift);
            hold on; plot(xDrift,yMarker,'ok','markersize',10)
            text(xDrift,yMarker,num2str(floor(t(iP)*c_p/L)),'fontsize',10,'VerticalAlignment','middle','HorizontalAlignment','center')
        end
    case 'stacked'
        ...
        hold on;box off;
        set(gca,'XAxisLocation','origin');
        dz = 2.5*max(abs(eta(1,:)));
        nPannel = length(t);
        for iP = 1:nPannel
            z0 = (iP-1)*dz;
            plot(res.x,eta(iP,:)+z0,'k','linewidth',1);
            text(L,z0,sprintf('t = %.2fs\nw_{nl} = %.2f',t(iP),nonLinRamp(t(iP))));
        ...
        % plot dashed line indicating phase velocity c_p
        tLineEnd = t(end);    TCut = L/c_p;
        nBreak = floor(tLineEnd/TCut);
        for iBreak = 1:nBreak
...
exportName = sprintf('%sT%.1f_ka%.2g_M%d_Nw%d_dt%.3gT_L%.0f_nx%d',exportPrefix,T,ka,M,NWaves,NT_dt,L,nx); exportName(exportName=='.')='p';
if DO_EXPORT
    copyfile('./test_HOS_SFo_basin.m',[exportPath,'/script_',exportName,'.m']) 
