...
limZeta = [-5*h2,7*h2,-pi,.5];
% limZeta = [-5*h2,5*h2,-pi+.001,0];
...
hp = patch([min(real(fz(:))).*[1,1],0,0,max(real(fz(:))).*[1,1]],[-1.1*max(h1,h2),-h1,-h1,-h2,-h2,-1.1*max(h1,h2)],.5*[1,1,1],'FaceAlpha',.5,'lineStyle','none');
...
% lambda = +exp(-zeta+1i*pi); % surface at imag(zeta) = 0, bed at imag(zeta) = -pi
% t = sqrt((lambda-c^2)./(lambda-1));
% % t = sqrt(lambda-c^2)./sqrt(lambda-1);
% fz = +1i*h2 - h1/pi.*(1/c.*log2((t-c)./(t+c))-log((t-1)./(t+1)));
% end
...
% t = sqrt((lambda-c^2)./(lambda-1));
% % t = sqrt(lambda-c^2)./sqrt(lambda-1);
% fz = +1i*h2 + h2/pi.*(1/c.*log((t-c)./(t+c))-log2((t-1)./(t+1)));
% end
