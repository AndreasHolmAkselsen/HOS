...
% nYInt = 500;
% % yi = linspace(-h,-wbl,nYInt)';
% % dL = (1-csc(pi*yi./(2*h)).^2.*sin(pi*wbl./(2*h)).^2).^(thetaMax/pi) /h ; 
...
% 
% for i = 1:10
%     yi = linspace(-h,-d,ny_minus)'/H;
%     dL = 2/pi*(1-csc(yi).^2.*sin(d/H).^2).^(thetaMax/pi) ; 
%     L = sum(.5*(dL(1:end-1)+dL(2:end)) .* diff(yi) )  ;
%     fprintf('L = %g, 1-wbl/h=%g\n',L,1-wbl/h)
% 
%     delta_d = wbl-h*(1-L);
%     d = d + delta_d;
% end
...
hp_BM = plot(nan(5,1),yWb,'o-k','LineWidth',2.5);
...
while t<=tMax
    ...
    hp_BM.XData = [0,0,-(wbl+wbOveWater)*tan(theta),0,0];
...
for i = 1:maxIt
    ...
    if abs(delta_d__h)<1e-6, break; end
    d__h = d__h + delta_d__h;
end
if i == maxIt
    warning('failed to find solution withing %d iterations. Last error: %g.',maxIt,delta_d__h)
end
end