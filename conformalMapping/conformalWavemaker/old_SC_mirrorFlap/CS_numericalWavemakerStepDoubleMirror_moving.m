...
while t<=tMax
    ...
    delete(hp_c)
    [~,hp_c(1)] = contour(real(z),imag(z),real(zz),30,'r','linewidth',1);
    [~,hp_c(2)] = contour(real(z),imag(z),imag(zz),30,'b','linewidth',1);
...
theta = -thetaMax;
z = fz(xx,yy,theta,wbl,wbOveWater);
contour(real(z),imag(z),real(zz),30,'r--','linewidth',1);
contour(real(z),imag(z),imag(zz),30,'b--','linewidth',1);  
...
h = -yy(end);
n = 2; % mirror n times
iMirror = shiftdim(-n:n,-1); 
df = prod(1+d^2./(zz+2i*iMirror*h).^2,3).^thp; % k=1
